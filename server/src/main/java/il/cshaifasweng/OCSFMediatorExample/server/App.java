...
*/
    ...
    public static void generateDataBase() throws Exception {
        ...
        Teacher t2 = new Teacher("Avi Cohen", "avi", false);       // new teacher
        t2.addSubject(sports);
        session.save(t2);
        session.flush();

        Teacher t3 = new Teacher("Snel Lechtman", "Boris_password", false);       // new teacher
        t3.addSubject(physics);
        session.save(t3);
        session.flush();

        Teacher t4 = new Teacher("Amir Smith", "Amir_password", false);       // new teacher
        t4.addSubject(mathematics);
        session.save(t4);
        session.flush();

        Teacher t5 = new Teacher("Roey Catz", "Roey_password", false);       // new teacher
        t5.addSubject(biology);
        session.save(t5);
        session.flush();

        Teacher t6 = new Teacher("Noam Levi", "Noam_password", false);       // new teacher
        t6.addSubject(chemestry);
        session.save(t6);
        session.flush();

        Teacher t7 = new Teacher("Roti Menashe", "Roti_password", false);       // new teacher
        t7.addSubject(history);
        session.save(t7);
        session.flush();

        Teacher t8 = new Teacher("Ben Mizrahi", "Ben_password", false);       // new teacher
        t8.addSubject(literature);
        session.save(t8);
        session.flush();

        Teacher t9 = new Teacher("Amnon Harel", "Amnon_password", false);       // new teacher
